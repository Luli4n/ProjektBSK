...
DWORD WINAPI ReceiveThreadFun(void *ptr)
{
	...
	Frame frame;
	unsigned long* ipHolder = new unsigned long;

	while (1)
	{
		int frame_size = pmt_net->reciv((char*)&frame, ipHolder, sizeof(Frame));   // oczekiwanie na nadejœcie ramki 
...
void VirtualWorldCycle()
{
	...
	{                              // nale¿y na nowo obliczyæ œredni czas cyklu avg_cycle_time
		...
		sprintf(text, "WZR-lab lato 2021/22 temat 2, program przykladowy (%0.0f fps  %0.2fms) ", fFps, 1000.0 / fFps);
	...
	frame.iID = my_car->iID;
	unsigned long ipTab[1] = { 0 };

	uni_send->send((char*)&frame, ServIP, sizeof(Frame));  // wys³anie komunikatu do pozosta³ych aplikacji
}
	...
	int       nCmdShow)
{
	...
	main_window = CreateWindow(class_name, "WZR-lab lato 2021/22 temat 2 - program przykladowy", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS,
